...
✅ React frontend     — Vite + TanStack Query + React Router; /login, /admin/dashboard, /pos

---

## Frontend (frontend/)
- Vite + React + TypeScript, dev server on port 5173
- Proxy: `/api/*` → `http://localhost:3000` (no CORS, cookies forwarded)
- Auth: JWT in module-level variable (not localStorage), silent refresh on mount via httpOnly cookie
- Pages: LoginPage, DashboardPage (admin/manager/warehouse/viewer), PosPage (cashier+)
